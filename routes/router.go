...
func SetUpRouter(c *fiber.App) {
	...
	r.Group("/carton/history").Get("", controllers.GetCartonHistory)
	...
	syncRoute.Get("/orderplan", controllers.GetSyncOrderPlan)
	...
	fileUpload.Patch("/invoice/fticket", controllers.GenerateFTicketInvoiceTap)
	...
	stock.Put("/data/:id", controllers.UpdateStockByID)
	stock.Delete("/data/:id", controllers.DeleteStockByID)

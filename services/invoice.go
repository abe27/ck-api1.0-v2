			...
			if i > 0 {
				if r, err := sheet.GetRow(i); err == nil {
					...
					if bhivno.GetString() != "" {
						...
						var invCount int64
						db.
							Where("bhivno=?", bhivno.GetString()).
							Where("bhodpo=?", bhodpo.GetString()).
							Where("bhivdt=?", etd.Format("2006-01-02")).
							Where("bhsafn=?", bhsafn.GetString()).
							Where("bhypat=?", bhypat.GetString()).

			...
			if i > 0 {
				if r, err := sheet.GetRow(i); err == nil {
					...
					if bhivno.GetString() != "" {
						...
						likeCust := "%" + bhsafn.GetString() + "%"
						if err := db.Raw(fmt.Sprintf("select id,bal_qty,bistdp from tbt_order_plans where bisafn like '%s' and etd_tap='%s' and part_no='%s' and shipment_id='%s' and pono in ('%s','%s') order by created_at desc,seq desc limit 1", likeCust, etd.Format("2006-01-02"), bhypat.GetString(), shipment.ID, strings.Trim(bhodpo.GetString(), ""), strings.Trim(strings.ReplaceAll(bhodpo.GetString(), " ", ""), ""))).Scan(&orderPlan).Error; err != nil {
							panic(err)
						}
